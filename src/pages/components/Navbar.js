...
const Navbar = ({ cart, addToCart, removeFromCart, clearCart, subtotal }) => {
  ...
  return (
    <div className=' flex flex-col md:flex-row md:justify-start sticky top-0 z-30 bg-white items-center justify-between py-2 shadow-xl'>
      ...
      <div className="cart absolute cursor-pointer  flex right-0 mx-6 top-4">
       <Link href={'/login'}> 
        <MdAccountCircle className='text-xl md:text-2xl mx-2' />
       </Link>
        <AiOutlineShoppingCart onClick={toggleCart} className='text-xl md:text-2xl' />
      </div>
      <div ref={ref} className={`w-72 h-[100vh] sidecart  absolute top-0 right-0 bg-pink-100 px-8 py-10 p-10  transform transition-transform  ${Object.keys(cart).length !== 0 ? 'translate-x-0' : 'translate-x-full'}  `}>
        ...
        <div className="flex float-left">
          <Link href={'/checkout'}> <button className="flex text-white bg-pink-500 border-0 px-3 py-2 mr-2  focus:outline-none hover:bg-pink-600 rounded text-sm mt-1 sm:mt-0"><BsFillBagCheckFill className='m-1' />Checkout</button></Link>
